#!/usr/bin/env python3
"""
Comfy Dispatcher Pipeline: Satirical Mexican Cat ICE Airport Rant
...
BRIEF_PATH = PROJECT / "output" / "briefs" / "satirical-mexican-cat-living-in-the-us-speaking-to-brief.json"
OUTPUT_DIR = PROJECT / "output" / "scenes" / "satirical-mexican-cat-living-in-the-us-speaking-to"
...
def build_video_prompt(scene):
    ...
    for d in scene.get("dialogue", []):
        ...
        dialogue_parts.append(
            f'The Mexican cat speaks directly to camera with {emotion} energy: "{line}"'
        )
    ...
    if dialogue_parts:
        parts.append(
            "Spoken dialogue (English with thick Mexican accent, passionate delivery): "
            + " | ".join(dialogue_parts)
        )
    if sfx_text:
...
async def main():
    log("=" * 60)
    log("COMFY DISPATCHER: Mexican Cat ICE Airport Rant 🇲🇽🐱")
    log("=" * 60)
    log(f"Scenes: {len(SCENES)}")
    log(f"Image: GeminiImage2Node (fallback: GeminiNanoBanana2)")
    ...
    log(f"Aspect ratio: {ASPECT} (selfie TikTok format)")
