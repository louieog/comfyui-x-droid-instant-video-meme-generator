#!/usr/bin/env python3
"""
Comfy Dispatcher Pipeline: Amazon Cat Goes Live
...
BRIEF_PATH = PROJECT / "output" / "briefs" / "a-hyperrealistic-orange-tabby-cat-wearing-an-amazo-brief.json"
OUTPUT_DIR = PROJECT / "output" / "scenes" / "a-hyperrealistic-orange-tabby-cat-wearing-an-amazo"
...
def build_scene_image_prompt(scene):
    """Build detailed image prompt from scene visual + character description."""
    visual = scene["visual"]
    prompt = f"{visual}\n\nCharacter details:\n{CHARACTER_DESC}"
    if len(prompt) > 4000:
        prompt = prompt[:4000]
...
def build_video_prompt(scene):
    ...
    The brief uses text overlay dialogue, but since KlingOmniPro supports
    native audio (generate_audio=true), we include dialogue and SFX
    descriptions in the prompt so the model generates ambient audio
    and atmosphere matching the scene.
    """
    camera = scene.get("camera", "")
    visual = scene["visual"]

    # Include dialogue for native audio — the cat "speaks" these lines
    # even though they're also text overlays
    dialogue_parts = []
    for d in scene.get("dialogue", []):
        line = d["line"]
        emotion = d.get("emotion", "")
        dialogue_parts.append(
            f'The cat speaks to camera with {emotion} energy: "{line}"'
        )
    ...
    if dialogue_parts:
        parts.append(
            "Audio — the cat narrates in a deep, deadpan male voice: "
            + " | ".join(dialogue_parts)
        )
    if sfx_text:
        parts.append(f"Sound effects: {sfx_text[:250]}")
    if music:
        parts.append(f"Music/atmosphere: {music[:200]}")
...
async def generate_tts_fallback(scene, scene_id):
    ...
    for i, d in enumerate(scene.get("dialogue", [])):
        ...
        save_workflow("tts", scene_id, workflow, line_idx=i)
...
async def main():
    log("=" * 60)
    log("COMFY DISPATCHER: Amazon Cat Goes Live 📦🐱🔴")
    log("=" * 60)
    log(f"Scenes: {len(SCENES)}")
    log(f"Image: GeminiImage2Node:gemini-3-pro-image-preview (fallback: GeminiNanoBanana2)")
    ...
    log(f"Aspect ratio: {ASPECT} (Instagram Live format)")
